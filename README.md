...
・タスクに写真を貼り付けたりやり終えたタスクを削除していきとても使いように構築ました。

・タスクにURLを入れるとそのページに飛ぶように考えました。

・タスクが多すぎても見づらくならないようにページ移動や、検索機能をつけました。

・ログイン機能もついているので自分だけのスケジュール管理ができます。

・rails c をすることで最初のユーザーを登録したり、そのユーザーのidを使ってタスクを100部増やすことができます。
...
・削除機能があることから何度も同じことをしないようにしています。

<h1>どんな課題や不便なことを解決するためにこのアプリを作ったのか。</h1>

・忘れごとが多い友達や高齢者の人に使っていたくと認知症対策や忘れごとが減っていくと思い制作しました。

・仕事とプライベートがすごく忙しくて大変な人に使ってもらいたいと思いました。

・優先順位を決めるのが苦手な人に使ってもらいたいと思いました。
...
・csvを導入する時に、うまくファイルを読み込めなかったので、ホームページで調べたりすることで改善することができました。

・最初ユーザー登録する時に、rails cでユーザーを作らないといけないことを知らなかったので、一から制作し直しました。

・ログインした時に、他のユーザーが書いたタスクが見えていたのでそれを改善するのに時間がかかりました。

・gemでslimを使っていたので、少しでもコードがずれていると構文エラーにつながったので、コードの起き位置を考える必要がありました。
...
・エラーが出るとなかなか解決できなかったら諦めてしまうところです。

・エラーが出ると解決できずにまた一から制作し直してしまいます。

・ToDoListにカレンダーをつけてみたいです。

・ToDoList意外にもホームページを自分で考えて制作してみたいです。
